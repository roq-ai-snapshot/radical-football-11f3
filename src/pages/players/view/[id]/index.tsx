...
import { deleteParentRequestById } from 'apiSdk/parent-requests';
...
function PlayerViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PlayerInterface>(
    ...
    () =>
      getPlayerById(id, {
        relations: ['user_player_user_idTouser', 'academy', 'user_player_coach_idTouser', 'parent_request'],
      }),
  );

  const parent_requestHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('parent_request', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Parent Requests
                    </Text>
                    <NextLink passHref href={`/parent-requests/create?player_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.parent_request?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/parent-requests/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.status}</Td>
                            <Td>
                              {hasAccess('parent_request', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/parent-requests/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('parent_request', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    parent_requestHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
